...
### TJMS case law for Claude, Cursor and AI agents

Search case law from the Mato Grosso do Sul state court (TJMS) straight from Claude, ChatGPT or your own agent, covering cível, consumidor e contratos. Every ruling comes with the judging body, rapporteur, date, the exact snippet that matched your query and the link to the official site. The same connection reaches 16 other Brazilian courts. Free, no login, hosted by the platform.

- ⚖️ **TJMS** plus 16 other Brazilian courts on the same connection
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Jurisprudência TJMS` and **URL** `https://api.mcp.ai/p_tjms`.

### Cursor

[➕ Install Jurisprudência TJMS in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=tjms&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF90am1zIn0=)

### VS Code (Copilot Chat)

[➕ Install Jurisprudência TJMS in VS Code](vscode:mcp/install?name=tjms&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_tjms%22%7D)
...
```
Recent TJMS rulings on consumer damages
Compare TJMS and the Superior Court on limitation periods
...
**O TJMS é o único tribunal?**
Não. A conexão serve 17 tribunais: além do TJMS, STF, STJ, TST, TRF3, TRF4, CARF e os demais tribunais de justiça estaduais. Restringir a busca ao TJMS é opcional, e comparar com as cortes superiores é uma pergunta só.
...
- 📧 [tjms@mcp.ai](mailto:tjms@mcp.ai)
- 🐛 [GitHub Issues](https://github.com/mcp-dir/tjms-mcp/issues)
- 📄 [docs/](docs/)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_tjms` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
